...
export default class videoEvent{
	...
	progress() {
		// 某些设备，前几次调用这个方法会报错
		try{
			let buffer = this._video.buffered.end(0);
	...
	showPlay() {
		this.hideMiddleElems();
		_.show([this._ui.middlePlay]);
	}
	showPause() {
		this.hideMiddleElems();
		_.show([this._ui.middlePause]);
	}
	progressTime() {
	...
	progressCss() {
		...
		let percent = this._video.currentTime? this._video.currentTime / this._video.duration * 100 + '%': 0;
